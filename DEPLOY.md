# 🚀 Deployment Guide: Anonymous Canvas Board
...
1.  **Just run this command** in your terminal to upload the code:
    ...
    *(If it asks for a username/password, use your GitHub credentials)*.
    ...
    > [!IMPORTANT]
    > **Before pushing:** Open `prisma/schema.prisma` and change:
...
2.  Import the GitHub repository you just created (`canvas-board`).
...
-   **Database Errors**: Ensure your `prisma/schema.prisma` provider is set to `postgresql` (I already did this for you).
